...
export default function ListingCard({ listing, index = 0, initialLiked = false, onLikeToggle, currentUserId }: {
  ...
  async function goChat(e: React.MouseEvent) {
    ...
    try {
      ...
      if (res.status === 400 && data.error === 'Own listing') { showToast('Это ваше объявление'); return }
  ...
  function openOffer(e: React.MouseEvent) {
    e.stopPropagation()
    if (isOwn) { showToast('Это ваше объявление'); return }
    setShowOffer(true)
  }
        ...
        <div style={{ padding: '14px 14px 12px' }}>
          ...
          {!isOwn ? (
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              {/* Оффер */}
              <button onClick={openOffer} style={{
                height: 38, borderRadius: 10, border: '1.5px solid #E0E1E6',
                background: '#F8F9FB', color: '#1A1C21',
            ...
            /* Свой товар — одна кнопка написать (disabled) */
