...
export default async function CounterpartiesPage() {
  ...
  // PERF FIX: 3 последовательных await → Promise.all (было ~600ms, стало ~200ms)
  // PERF FIX: select('*') → явные поля (меньше трафик)
  const [
    { data: counterparties },
    { data: dealCounts },
    { data: allPayments },
        ...
        supabase
          .from('counterparties')
          .select('id, name, company, type, phone, notes, dexa_profile_id, created_at')
          .eq('owner_id', user!.id)
  ...
  const ordersTotalMap: Record<string, number> = {}
      const dealsMap: Record<string, number> = {}
          for (const d of dealCounts ?? []) {
                ...
                const price =
                        d.counter_status === 'accepted' && d.counter_price
                    ? d.counter_price
                          : d.total_price
  ...
  return <CounterpartyList counterparties={enriched} />
}
